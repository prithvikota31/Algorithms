...
| 22 | ☑ | Given rectangles on a plane, find a vertical line that divides their total area equally, counting overlapping rectangle area separately. → [sweep-line solution](22-rectangle-area-split/VerticalAreaSplit.java) (sum active rectangle heights between x-events). **<span style="color:red">Union-area follow-up marked optional/conceptual only (not implemented): if overlap must count once, maintain active y-interval union length with coordinate compression + segment tree.</span>** |
